...
puzzle:     .byte 0:400
    ...
    # canvas 6 words + 1 char with padding
    #.word 1     # unsigned int height
    #.word 1     # unsigned int width
    #.byte 1     # unsigned char pattern
    #.word 1     # char** canvas
    #.word 1     # int num_lines
    #.word 2     # unsigned int* coords[2]

    # data 300 bytes
    #.word 24    # int lin_data[2 * MAX_LINES = 24]
    #.word 12    # char* canvas_row_pointer[MAX_DIM = 12]
    #.byte 156   # char canvas_data[MAX_DIM * (MAX_DIM + 1) = 156]
...
solve_puzzle:      
    ...
    lb $t0, 0($t0)
    li $t1, 30
    beq $t0, $t1, charge_and_shoot          # get 50 bullets
    ...
    la $s0, puzzle          # PuzzleWrapper puzzle
    la $s1, solution        # Solution sol
    la $s2, puzzle_flag     # puzzle_flag
    li $t3, 0
    sb $t3, 0($s2)       

    sw $s0, REQUEST_PUZZLE      # calls to get puzzle
    li $t0, 1
checker:                    # waits for puzzle_flag to be 1
    lb $t2, puzzle_flag          # puzzle_flag   
    beq $t2, $t0, done_checking
    j checker

done_checking:
    add $t0, $s0, 16               
    move $a0, $t0               # Lines * lines
    move $a1, $s0               # Canvas * canvas
    move $a2, $s1               # Solution solution
...
request_puzzle_interrupt:
    ...
    la $t0, puzzle_flag
    li $t1, 1
    sb $t1, 0($t0)             # raises puzzle flag, puzzle_flag = 1
