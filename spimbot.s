...
for_2:
    lb $t0 0($s0)
    beq $t0 1 exit_2                            # exit the loop once we've bonked
    jal solve_puzzle
    j for_2
exit_2:
    li $t0 1
...
for_3:
    lb $t0 0($s0)
    beq $t0 1 exit_3                            # exit the loop once we've bonked
    jal solve_puzzle
    j for_3
exit_3:
...
for_4:
    lb $t0 0($s0)
    beq $t0 1 exit_4                            # exit the loop once we've bonked
    jal solve_puzzle
    j for_4
exit_4:
...
solve_puzzle:
    ...
    li $s3 0				# #s3 = the value of puzzle flag
